var client = require("./redis").client,
    ...
    buildGrammar = function(name, title, body, revision) {
        return {
            name: name,
            title: title,
            grammar: body,
...
module.exports = injectable(client, function(client, done) {
    var fetchLatestRevisionNumberByName = function(name, onSuccess, onFailure) {
            client.get(keys.forGrammar(name).revision, function(error, value) {
                if (value === null) {
                    onFailure();
                } else {
                    onSuccess(parseInt(value.toString()));
                }
            });
        },
        fetchByNameAndRevision = function(name, revision, onSuccess, onFailure) {
            ...
            client.mget(grammarKeys.title, bodyKey, function(error, value) {
                var title = value[0] && value[0].toString(),
                    body = value[1] && value[1].toString();
                if (body === null) {
                    onFailure();
                    return;
                }
                onSuccess(buildGrammar(name, title, body, revision));
            });
        },
        ...
        fetchByNameOrDefault = function(name, onSuccess, onFailure) {
            ...
            client.mget(grammarKeys.title, grammarKeys.revision, function(error, value) {
                ...
                if (revision === null) {
                    onSuccess(buildGrammar(name, title, null));
                    return;
                }
                client.get(grammarKeys.forRevision(revision).body, function(error, body) {
                    onSuccess(buildGrammar(name, title, body.toString()));
                });
            });
        };
    done({
