...
http404Handler = injectable(content, function(content) {
    content("404", {}, 404);
});

aboutHandler = injectable(content, function(content) {
    content("about");
});
...
http.createServer(function(request, response) {
    ...
    sessions(request, response, function() {
        ...
        dispatcher.add(/^\/grammars\/([a-z0-9\-_]+)\/$/, "name", grammarHandlers.show);
        ...
        dispatcher.add(/^\/grammars\/([a-z0-9\-_]+)\/revision\/([0-9]+)\/sentence\/([0-9\-]+)\/$/,
            "name", "revision", "sequence", grammarHandlers.show);
        dispatcher.add(/^\/grammars\/([a-z0-9\-_]+)\/source\/$/, "name", grammarHandlers.source);
        dispatcher.add(/^\/grammars\/([a-z0-9\-_]+)\/revision\/([0-9]+)\/source\/$/,
            "name", "revision", grammarHandlers.source);
        dispatcher.add(/^\/grammars\/([a-z0-9\-_]+)\/edit\/$/, "name", grammarHandlers.edit);
        dispatcher.add(/^\/edit-my-grammar\//, grammarHandlers.editLoggedInUsersGrammar);
        
        dispatcher.add(/^\/register\/$/, require("./controllers/account/register"));
        dispatcher.add(/^\/login\/$/, require("./controllers/account/login"));
        dispatcher.add(/^\/logout\/$/, require("./controllers/account/logout"));
