...
exports.userRepository = injectable("request", function(request, done) {
    var keyForUsername = function(username) {
        ...
        logout: function(callback) {
            // FIXME: shouldn't be playing around with the request here
            if (request.session) {             
                client.del(keyForUsername(request.session.userId)("authToken"), function(error, result) {
        ...
        fetchDisplayNamesFor: function(names, callback) {
            var queryArguments = names.map(function(name) {
                return keyForUsername(name)("realname");
            });
            queryArguments.push(function(error, titles) {
                callback((titles || []).map(function(title) {
