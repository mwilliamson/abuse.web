...
BLOBS.create = function(id) {
    ...
    function PointMass(cx, cy, mass) {
        ...
        this.draw = function (ctx, scaleFactor) {
            ...
            ctx.beginPath();
            ctx.arc(this.cur.x * scaleFactor, this.cur.y * scaleFactor, 4.0, 0.0, Math.PI * 2.0, true);
            ctx.fill();
        };
    }
    ...
    function BlobCollective(x, y, startNum, maxNum) {
        ...
        this.selectBlob = function (x, y) {
            var i, aXbX, aYbY, dist, minDist = 10000.0;
            var otherPointMass;
        ...
        this.addForce = function (force) {
            ...
            for (i = 0; i < this.blobs.length; i++) {
                ...
                this.tmpForce.x = force.x * (Math.random() * 0.75 + 0.25);
                this.tmpForce.y = force.y * (Math.random() * 0.75 + 0.25);
    ...
    function init() {
        ...
        function getMouseCoords(event) {
            ...
            if (event.pageX || event.pageY) {
                return {
                    x: (event.pageX - $(canvas).offset().left) / scaleFactor,
                    y: (event.pageY - $(canvas).offset().top) / scaleFactor
                };
            }
            return null;
        }
    ...
    return {
        stop: stop,
        start: start,
        bindControlsTo: function(element) {
